import { processBillingCycle } from '../jobs/billing';
...
async function test() {
    ...
    try {
        ...
        if (!setting) {
            console.log('No settings found'); return;
        }
        ...
        if (userAfter1 && userAfter1.coins < 10) console.log('✅ Billing Successful');
        else console.log('❌ Billing Failed');

        // --- TEST 2: Insufficient Coins (Suspend) ---
        console.log('\n--- TEST 2: Insufficient Coins (Auto-Suspend) ---');
        const tooLittle = costPerMin * 0.5;
        await prisma.user.update({ where: { id: userId }, data: { coins: tooLittle } });

        // Reset server status to active first?
        await prisma.server.update({ where: { id: serverId }, data: { isSuspended: false, status: 'active' } });

        await processBillingCycle();

        const serverAfter2 = await prisma.server.findUnique({ where: { id: serverId } });
        console.log(`Server Status: ${serverAfter2?.status}, IsSuspended: ${serverAfter2?.isSuspended}`);
        if (serverAfter2?.isSuspended && serverAfter2?.suspendReason === 'INSUFFICIENT_COINS') console.log('✅ Auto-Suspend Successful');
        else console.log('❌ Auto-Suspend Failed');

        // --- TEST 3: Auto-Resume ---
        console.log('\n--- TEST 3: Auto-Resume ---');
        await prisma.user.update({ where: { id: userId }, data: { coins: 100 } });

        await processBillingCycle(); // Should resume

        const serverAfter3 = await prisma.server.findUnique({ where: { id: serverId } });
        console.log(`Server Status: ${serverAfter3?.status}, IsSuspended: ${serverAfter3?.isSuspended}`);
        if (!serverAfter3?.isSuspended && serverAfter3?.status === 'active') console.log('✅ Auto-Resume Successful');
        else console.log('❌ Auto-Resume Failed');
