...
exports.playSlots = exports.playCoinFlip = exports.playDice = void 0;
const prisma_1 = require("../prisma");
// Helper to update balance
const updateBalance = async (userId, amount, type, description, balanceAfter) => {
...
const playDice = async (req, res) => {
    try {
        ...
        if (!prediction || prediction < 1 || prediction > 6)
            return res.status(400).json({ message: 'Invalid prediction (1-6)' });
...
const playCoinFlip = async (req, res) => {
    try {
        ...
        if (!['heads', 'tails'].includes(choice))
            return res.status(400).json({ message: 'Invalid choice (heads/tails)' });
        const user = await prisma_1.prisma.user.findUnique({ where: { id: userId } });
        if (!user || user.coins < betAmount)
            return res.status(400).json({ message: 'Insufficient coins' });
        // Logic
